...
if (otlpServer) {
    ...
    otelSdk = new NodeSDK({
        traceExporter: new OTLPTraceExporter(collectorOptions), // <-- This seems to be the culprit. Replacing it with a ConsoleSpanExporter makes stuff print to console just fine
        metricReader: new PeriodicExportingMetricReader({
            exportIntervalMillis: environment === 'development' ? 5_000 : 10_000,
            exporter: new OTLPMetricExporter(collectorOptions),
        }),
        logRecordProcessor: new SimpleLogRecordProcessor(new OTLPLogExporter(collectorOptions)),
        instrumentations: [
